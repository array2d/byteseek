//! rwir `kvlanglayout.*`：corebrain 自造 kv 代码入库（直连 layout 的 C ABI）。
//!   kvlanglayout.vet(src)    -> "ok" | 错误信息      只校验（parse+lower），不写 kvspace
//!   kvlanglayout.src(src)    -> entry | "error: …"   把内存源码 layout 进 kvspace（LLM 生成即插入）
//!   kvlanglayout.layout(path)-> entry | "error: …"   从 .kv 文件 layout 进 kvspace
//! 三者都在 C 边界 catch_unwind：坏代码返回 -1，绝不打崩 byteseek 进程。
